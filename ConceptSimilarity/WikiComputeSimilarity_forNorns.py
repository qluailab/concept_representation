...
　　该函数由ProcWikiParsedOutToTrainCorpus.py拷贝过来.  已有修改，添加了pickle保存变量与读取变量的功能，能有效加快读取速度．
    ...
    if hostName.rfind('zero') != -1:  # 这意味着当前程序正在我的本地服务器上执行
        ...
        path = '/home/ailab/wiki/Datas/WikiExtracter/enwiki-20161001-pages-articles-multistream-index_IdTitleRedirectNamespaceLemma-Tab-Finalredirect-LowerCompactTitle.txt'
...
def getAutorunScoreOrderedbyWordpairs(dict_wordpair2autorunscore, list_wordpairs, dataSet):
    ...
    if dataSet == DataSet.WordSim353:
        assert len(list_wordpairs) ==349 and len(list_wordpairs) == len(dict_wordpair2autorunscore) + 1, u'{}: the number should be 353:352, now {}:{}' \
...
def getHumanScoreOrderedbyWordpairs(dict_wordpair2humanscore, list_wordpairs, dataSet):
    ...
    ret = []
    if dataSet == DataSet.WordSim353:
        assert len(list_wordpairs) == len(dict_wordpair2humanscore.keys())+1 and len(list_wordpairs) == 349, u'{}: the number should 353 vs 352. {}:{}'\
...
def GetAndCheckDataFile(dataSet):
    ...
    elif dataSet == DataSet.MC30:
        excelScorePath = gl_mc30path
        conceptPath = gl_conceptmc30path
    else:
...
def CheckFormatandDatainExcelfile(ws, dataSet):
    ...
    if dataSet == DataSet.WordSim353:
        row1="51"  # located the repeated line of 'professor' 'cucumber'
        row2 = "368" # last line
        assert ws.cell("A"+row1).value == u'professor' and ws.cell(
            "B"+row1).value == u'cucumber', u'{}: Please assure the content of wordsim353score.xlsx is right!'.format(
            get_cur_info())
        ...
        row1 = 20
        row2 = 933
        assert ws.cell("C" + str(row1)).value == 8.11 and ws.cell("C" + str(row2)).value == 7.33, \
...
def HandleInputParameterDataSet(strDataset, list_dataSet, list_dataSetexcelfile):
    ...
    if strDataset == u'All':
        ...
        list_dataSet.append(DataSet.MC30)
        list_dataSetexcelfile.append(os.path.basename(gl_mc30path))
